...
export const createCustomReleaseRepository = (
  ...
  return {
    ...
    owner: { login: normalized.owner, avatar_url: `https://github.com/${normalized.owner}.png` },
    source_added_at: now,
  };
};
...
const normalizeCustomReleaseRepo = (value: unknown, sourceId: ReleaseSourceId): CustomReleaseRepository | null => {
  ...
  return {
    id: typeof record.id === 'number' && Number.isFinite(record.id) ? record.id : getLocalReleaseRepoId(parsed.full_name, sourceId),
    ...
    owner: {
      ...
      avatar_url: typeof ownerRecord.avatar_url === 'string' && ownerRecord.avatar_url ? ownerRecord.avatar_url : `https://github.com/${parsed.owner}.png`,
    },
...
export const normalizeCustomReleaseRepositories = (value: unknown, sourceId: ReleaseSourceId): CustomReleaseRepository[] => {
...
export const normalizeReleaseSourceSettings = (value: unknown): ReleaseSourceSettings => {
  ...
  return {
    enabledSourceIds: enabledSourceIds.length > 0 ? Array.from(new Set(enabledSourceIds)) : defaultReleaseSourceSettings.enabledSourceIds,
    watchCustomReleaseRepos: normalizeCustomReleaseRepositories(record.watchCustomReleaseRepos, WATCH_CUSTOM_RELEASE_SOURCE_ID),
    customReleaseRepos: normalizeCustomReleaseRepositories(record.customReleaseRepos, CUSTOM_RELEASE_SOURCE_ID),
  };
};

export const mergeReleaseSourceSettings = (current: ReleaseSourceSettings, incoming: ReleaseSourceSettings): ReleaseSourceSettings => {
  const mergeRepos = (a: CustomReleaseRepository[], b: CustomReleaseRepository[]) => {
    const merged = new Map<string, CustomReleaseRepository>();
    [...a, ...b].forEach(repo => { const key = normalizeRepoKey(repo.full_name); if (!merged.has(key)) merged.set(key, repo); });
...
export const resolveReleaseSources = (
  ...
  const addRepository = (repository: Repository, sourceId: ReleaseSourceId) => {
    ...
    if (existing) { if (!existing.sources.includes(sourceId)) existing.sources.push(sourceId); return; }
    entriesByKey.set(key, { repository, sources: [sourceId] });
  };
  if (enabled.has(STARRED_RELEASE_SOURCE_ID)) {
    state.repositories.filter(repo => state.releaseSubscriptions.has(repo.id)).forEach(repo => addRepository(repo, STARRED_RELEASE_SOURCE_ID));
  }
  if (enabled.has(WATCH_CUSTOM_RELEASE_SOURCE_ID)) {
    settings.watchCustomReleaseRepos.filter(repo => !repo.release_hidden).forEach(repo => addRepository(customReleaseRepositoryToRepository(repo, WATCH_CUSTOM_RELEASE_SOURCE_ID), WATCH_CUSTOM_RELEASE_SOURCE_ID));
  }
  if (enabled.has(CUSTOM_RELEASE_SOURCE_ID)) {
    settings.customReleaseRepos.forEach(repo => addRepository(customReleaseRepositoryToRepository(repo, CUSTOM_RELEASE_SOURCE_ID), CUSTOM_RELEASE_SOURCE_ID));
  }
  const entries = Array.from(entriesByKey.values());
  return { entries, repositories: entries.map(entry => entry.repository), enabledSourceIds: settings.enabledSourceIds };
};
...
export const getSourcesForReleaseRepository = (
  ...
  if (state.repositories.some(repo => normalizeRepoKey(repo.full_name) === repoKey && state.releaseSubscriptions.has(repo.id))) sources.push(STARRED_RELEASE_SOURCE_ID);
  if (settings.watchCustomReleaseRepos.some(repo => normalizeRepoKey(repo.full_name) === repoKey)) sources.push(WATCH_CUSTOM_RELEASE_SOURCE_ID);
  if (settings.customReleaseRepos.some(repo => normalizeRepoKey(repo.full_name) === repoKey)) sources.push(CUSTOM_RELEASE_SOURCE_ID);
  return sources;
};

export const releaseBelongsToResolvedSources = (release: Release, resolved: ResolvedReleaseSources): boolean => {
