...
export const ReadmeModal: React.FC<ReadmeModalProps> = ({
  ...
  const fetchReadmeContentFromAvailableSource = useCallback(async (
    ...
    const fetchFromGitHubApi = async () => {
      if (!githubToken) {
        throw new Error(language === 'zh' ? 'Cloudflare Worker 不可用，无法加载 README' : 'Cloudflare Worker is unavailable, cannot load README');
      }
    ...
    try {
      ...
      console.warn('Falling back to the generic Worker GitHub proxy after README route failure:', backendError);
      return fetchFromGitHubApi();
    }
  ...
  const fetchReadmeCandidatesFromAvailableSource = useCallback(async (
    ...
    try {
      ...
      console.warn('Falling back to generic Worker-proxied README detection after route failure:', backendError);
      return fetchFromGitHubApi();
    }
  }, [githubToken]);
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      ...
      >
        <div
          ...
          tabIndex={-1}
          className="relative flex max-h-[90vh] w-full transform flex-col rounded-[1.5rem] border border-slate-200/80 bg-white shadow-[0_36px_110px_rgba(15,23,42,0.28)] transition-all dark:border-white/[0.08] dark:bg-[#101827]"
