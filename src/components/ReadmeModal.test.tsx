...
const mockedCreateGitHubApiService = createGitHubApiService as unknown as ReturnType<typeof vi.fn>;
...
describe('ReadmeModal multilingual README switching', () => {
  beforeEach(() => {
    ...
    mockedCreateGitHubApiService.mockImplementation(() => mockGitHubApi);
  ...
  it('falls back to the Worker GitHub proxy when the specialized README route fails', async () => {
    ...
    try {
      ...
      expect(mockedCreateGitHubApiService).toHaveBeenCalledWith();
  ...
  it('does not use the fallback Worker route when the backend reports README is missing', async () => {
    setMockStore('github-token');
  ...
  it('falls back to the Worker GitHub proxy when a selected README path fails', async () => {
    const consoleWarnSpy = vi.spyOn(console, 'warn').mockImplementation(() => undefined);
    try {
      setMockStore('github-token');
  ...
  it('falls back to Worker-proxied README variant detection when candidate listing fails', async () => {
    const consoleWarnSpy = vi.spyOn(console, 'warn').mockImplementation(() => undefined);
    try {
      setMockStore('github-token');
