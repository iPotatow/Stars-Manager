...
export const GistCard: React.FC<GistCardProps> = ({
  ...
  const handleAnalyze = async (event: React.MouseEvent) => {
    ...
    try {
      ...
      toast(t('Gist AI 分析完成', 'Gist AI analysis complete'), 'success');
    } catch (error) {
      updateGist({
        ...gist,
        analyzed_at: new Date().toISOString(),
      ...
      toast(t('Gist AI 分析失败', 'Gist AI analysis failed'), 'error');
    } finally {
  ...
  const handleDelete = async (event: React.MouseEvent) => {
    ...
    try {
      ...
      toast(error instanceof Error ? error.message : t('删除 Gist 失败', 'Failed to delete gist'), 'error');
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex shrink-0 items-center gap-1">
          <button
            ...
            className="rounded-lg p-2 text-gray-500 transition-colors hover:bg-brand-indigo/10 hover:text-brand-indigo disabled:opacity-50 dark:text-text-tertiary dark:hover:bg-brand-indigo/[0.15] dark:hover:text-white"
          ...
          {isMine && (
            <>
              <button type="button" onClick={(event) => { event.stopPropagation(); onEdit(gist); }} className="rounded-lg p-2 text-gray-500 hover:bg-light-surface" title={t('编辑', 'Edit')}><Edit3 className="h-4 w-4" /></button>
              <button type="button" onClick={handleDelete} disabled={isMutating} className="rounded-lg p-2 text-gray-500 hover:bg-red-50 disabled:opacity-50" title={t('删除', 'Delete')}><Trash2 className="h-4 w-4" /></button>
            </>
          )}
        </div>
      </div>
