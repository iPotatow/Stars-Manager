...
const SelectionAwareButton: React.FC<SelectionAwareButtonProps> = ({
  ...
  const baseClasses = 'inline-flex h-8 w-8 items-center justify-center rounded-xl border border-transparent p-0 text-slate-500 transition duration-200 hover:border-slate-200/80 hover:bg-white hover:text-slate-950 hover:shadow-sm disabled:opacity-50 dark:text-slate-400 dark:hover:border-white/[0.08] dark:hover:bg-white/[0.06] dark:hover:text-white';
  ...
  const variantClasses = {
    ...
    edit: 'bg-slate-100/80 dark:bg-white/[0.04]',
...
const RepositoryCardComponent: React.FC<RepositoryCardProps> = ({
  ...
  // 向量搜索是否可用（开关开启 + 当前 Worker 的 Vectorize 绑定已连接 + 有索引 + Embedding 配置完整）
  ...
  const handleAIAnalyze = async () => {
    ...
    if (!activeConfig) {
      toast(t('请先在设置中配置 AI 服务。', 'Configure an AI service in Settings first.'), 'error');
      return;
    }

    if (activeConfig.apiKeyStatus === 'decrypt_failed' || activeConfig.apiKeyStatus === 'empty') {
      toast(t('AI 服务的 API 密钥不可用，请在设置中重新输入并保存。', 'The AI service API key is unavailable. Re-enter and save it in Settings.'), 'error');
      return;
    }

    if (!activeConfig.baseUrl || !activeConfig.apiKey || !activeConfig.model) {
      toast(t('AI 服务配置不完整，请检查 API 端点、密钥和模型名称。', 'The AI service configuration is incomplete. Check the endpoint, key, and model.'), 'error');
      return;
    }
    ...
    try {
      ...
      const successMessage = repository.analyzed_at
        ? (language === 'zh' ? 'AI 重新分析完成' : 'AI re-analysis complete')
        : (language === 'zh' ? 'AI 分析完成' : 'AI analysis complete');
      ...
      if (!controller.signal.aborted) {
        ...
        const errorMsg = error instanceof Error && error.message
          ? error.message
          : (language === 'zh' ? 'AI 分析失败，请检查配置和网络连接' : 'AI analysis failed. Check the configuration and network.');
        ...
        toast(language === 'zh' ? 'AI 分析失败，请检查配置和网络连接。' : 'AI analysis failed. Check the configuration and network.', 'error');
      }
    } finally {
  ...
  return (
    ...
    >
      {repository.language ? (
        <span
          aria-hidden="true"
          className="absolute inset-y-5 left-0 w-0.5 rounded-r-full opacity-80"
      ...
      <div className="mb-3 flex items-start space-x-3">
        <img
          ...
          className="h-9 w-9 flex-shrink-0 rounded-xl object-cover ring-1 ring-slate-950/10 dark:ring-white/10"
        />
        <div className="min-w-0 flex-1">
          <h3 className="truncate text-[15px] font-bold tracking-[-0.02em] text-slate-950 dark:text-white">
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          <SelectionAwareButton
            ...
            className={`${
              ...
              ? 'bg-rose-500/[0.08] text-rose-600 hover:bg-rose-500/[0.12] dark:text-rose-300'
              : repository.analyzed_at
                ? 'bg-brand-indigo/[0.09] text-brand-violet hover:bg-brand-indigo/[0.14] dark:bg-brand-indigo/20 dark:text-indigo-200'
          ...
          <SelectionAwareButton
            ...
            className={`${isSubscribed
              ? 'border-brand-indigo/20 bg-brand-indigo text-white shadow-press dark:text-white'
              : 'bg-slate-100/80 text-slate-500 dark:bg-white/[0.04] dark:text-slate-400'
            }`}
        ...
        <div className="flex items-center gap-1.5">
          <a
            ...
            className={`inline-flex h-8 w-8 items-center justify-center rounded-xl bg-brand-indigo/[0.08] text-brand-violet transition hover:bg-brand-indigo/[0.14] dark:bg-brand-indigo/[0.15] ${selectionMode ? 'pointer-events-none opacity-50' : ''}`}
          ...
          <a
            ...
            rel="noopener noreferrer"
            onClick={(e) => selectionMode && e.preventDefault()}
            className={`inline-flex h-8 w-8 items-center justify-center rounded-xl bg-slate-100/80 text-slate-500 transition hover:bg-white hover:text-slate-950 hover:shadow-sm dark:bg-white/[0.04] dark:text-slate-400 dark:hover:bg-white/[0.07] dark:hover:text-white ${selectionMode ? 'pointer-events-none opacity-50' : ''}`}
      ...
      <div className="mb-4 flex-1">
        ...
        >
          <p
            className="mb-2 line-clamp-3 rounded text-[13px] font-medium leading-[1.65] text-slate-600 transition-colors duration-200 hover:text-slate-950 dark:text-slate-300 dark:hover:text-white"
          >
        ...
        <div className="flex items-center space-x-1 text-xs text-status-red dark:text-status-red" title={language === 'zh' ? 'AI 分析失败，点击 AI 按钮重试' : 'AI analysis failed. Click the AI button to retry'}>
              ...
              <div className="group relative">
                ...
                <div className="absolute left-0 top-full mt-2 w-72 max-w-xs p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white text-xs rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-[9999] whitespace-normal break-words">
                  <p className="text-gray-600 dark:text-gray-300 leading-relaxed">
                    {repository.analysis_error || (language === 'zh' ? 'AI 分析失败，请检查配置和网络连接' : 'AI analysis failed. Check the configuration and network.')}
                  </p>
      ...
      {displayTags.tags.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {displayTags.tags.map((tagItem, index) => (
            <span
              key={`tag-${index}`}
              className="rounded-lg border border-slate-200/70 bg-slate-50 px-2 py-1 text-[11px] font-semibold text-slate-500 dark:border-white/[0.06] dark:bg-white/[0.035] dark:text-slate-400"
            >
          ...
          <div className="flex space-x-1">
            {repository.ai_platforms.slice(0, 6).map((platform, index) => {
              ...
              return (
                <div
                  key={index}
                  className="flex h-6 w-6 cursor-default items-center justify-center rounded-lg bg-slate-100/80 text-slate-400 transition-colors hover:text-slate-700 dark:bg-white/[0.04] dark:text-slate-500 dark:hover:text-slate-200"
                  title={displayName}
                >
      ...
      <div className="space-y-3 mt-auto">
        {/* Language and Stars */}
        <div className="flex items-center space-x-4 text-[11px] font-semibold text-slate-400 dark:text-slate-500">
          {repository.language && (
        ...
        <div className="flex items-center justify-between border-t border-slate-200/70 pt-3 text-[11px] font-medium text-slate-400 dark:border-white/[0.06] dark:text-slate-500">
