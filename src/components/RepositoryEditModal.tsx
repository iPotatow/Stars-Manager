      ...
      <div className="space-y-5">
        ...
        <div className={sectionClass}>
          ...
          >
            ...
            <optgroup label={t('默认分类', 'Default Categories')}>
              {allCategories.filter(category => category.id !== 'all' && !category.isCustom).map(category => (
          ...
          {editIntent.category === 'keep-custom' && formData.category && (
            <div className={infoBoxClass}>
              <p className={`${infoTextClass} text-brand-violet`}>
                ...
                <span>
                  {t(
                    '已选择自定义分类。保存后仓库会固定在此分类中，不会随 AI 分析结果变化。开启分类锁定后，同步时也不会覆盖此设置。',
                    'Custom category selected. After saving, the repository will stay in this category and will not follow AI analysis results. Enable category lock to keep sync from changing it.'
                  )}
                </span>
              </p>
            </div>
