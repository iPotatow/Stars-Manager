...
export const RepositoryList: Vue.FC<RepositoryListProps> = ({
  ...
  const handleAIAnalyze = async (analyzeUnanalyzedOnly: boolean = false, analyzeFailedOnly: boolean = false) => {
    ...
    if (!activeConfig) {
      toast(language === 'zh' ? '请先在设置中配置 AI 服务。' : 'Configure an AI service in Settings first.', 'error');
      return;
    }

    if (activeConfig.apiKeyStatus === 'decrypt_failed' || activeConfig.apiKeyStatus === 'empty') {
      toast(language === 'zh' ? 'AI 服务的 API 密钥不可用，请在设置中重新输入并保存。' : 'The AI service API key is unavailable. Re-enter it in Settings and save the configuration.', 'error');
      return;
    }

    if (!activeConfig.baseUrl || !activeConfig.apiKey || !activeConfig.model) {
      toast(language === 'zh' ? 'AI 服务配置不完整，请检查 API 端点、密钥和模型名称。' : 'The AI service configuration is incomplete. Check the endpoint, key, and model name.', 'error');
      return;
    }
    ...
    try {
      ...
      const completionMessage = shouldStopRef.current
        ? (language === 'zh'
            ? `AI 分析已停止：成功 ${successCount} 个，失败 ${failedCount} 个`
            : `AI analysis stopped: ${successCount} succeeded, ${failedCount} failed.`)
        : (language === 'zh'
            ? `AI 分析完成：成功 ${successCount} 个，失败 ${failedCount} 个（平均响应 ${stats.averageResponseTime} ms）。`
            : `AI analysis complete: ${successCount} succeeded, ${failedCount} failed (average response ${stats.averageResponseTime} ms).`);
    ...
    try {
      switch (action) {
        ...
        case 'ai-summary': {
          ...
          if (!await confirm(t('确认 AI 分析', 'Confirm AI analysis'), confirmMessage, { type: 'warning' })) return;
          ...
          try {
            ...
            await forceSyncToBackend();
            toast(language === 'zh'
              ? `分析完成：成功 ${successCount} 个，失败 ${failedCount} 个（平均响应 ${stats.averageResponseTime}ms）`
            ...
            toast(language === 'zh' ? '批量 AI 分析失败。' : 'Bulk AI analysis failed.', 'error');
          } finally {
            // 确保状态重置
            optimizerRef.current = null;
            isAnalyzingRef.current = false;
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:gap-4">
          ...
          <div className="relative">
            ...
            {showDropdown && !isLoading && (
              <div className="absolute left-0 top-full z-30 mt-2 w-60 overflow-hidden rounded-xl border border-slate-200/90 bg-white p-1.5 shadow-[0_20px_55px_rgba(15,23,42,0.18)] dark:border-white/[0.08] dark:bg-[#151e2f]">
                <button
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
              ...
              <div className="flex items-center space-x-3 sm:space-x-4">
                <label 
                  ...
                  title={hasAnalyzedRepos ? t('显示 AI 生成的分析总结', 'Show the AI-generated summary') : t('当前没有 AI 分析内容', 'No AI analysis is available')}
                >
                  <input
                    type="radio"
                    name="displayContent"
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                {analyzedCount > 0 && (
                  <span className="text-xs sm:text-sm">
                    • {analyzedCount} {t('个仓库已完成 AI 分析', 'repositories analyzed')}
                  </span>
                )}
                {failedCount > 0 && (
                  <span className="text-xs sm:text-sm">
      ...
      <div
        className="grid min-h-[200px] grid-cols-1 gap-4 md:grid-cols-2 2xl:gap-5"
        onClick={handleClick}
