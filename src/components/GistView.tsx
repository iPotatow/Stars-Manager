...
export const GistView: React.FC = () => {
  ...
  const analyzeVisibleGists = async () => {
    ...
    if (!activeConfig.baseUrl || !activeConfig.apiKey || !activeConfig.model || activeConfig.apiKeyStatus === 'decrypt_failed' || activeConfig.apiKeyStatus === 'empty') {
      toast(t('AI 服务配置不完整，请检查设置。', 'The AI service configuration is incomplete. Check Settings.'), 'error');
      return;
    }
    ...
    toast(t(`AI 分析完成：成功 ${success}，失败 ${failed}`, `AI analysis complete: ${success} succeeded, ${failed} failed`), failed > 0 ? 'error' : 'success');
  };
  ...
  const handleSubmitGist = async (input: GistCreateInput | GistUpdateInput) => {
    try {
      ...
      toast(error instanceof Error ? error.message : t('Gist 保存失败', 'Failed to save gist'), 'error');
    }
  };
              ...
              <div className="group relative">
                ...
                <div className="absolute left-0 top-full z-[9999] mt-2 w-72 max-w-xs whitespace-normal rounded-lg border border-gray-200 bg-white p-3 text-xs text-gray-600 shadow-lg opacity-0 invisible transition-all break-words group-hover:visible group-hover:opacity-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
                  ...
                  <p className="leading-relaxed">
                    {t(
                      '若私有 gist 未拉取到，请到 GitHub → Settings → Developer settings → Personal access tokens 中确认当前 token 已勾选 gist 权限。修改权限后请重新登录。',
                      'If your private gists are missing, make sure the gist scope is checked for the current token, then sign in again.'
                    )}
                  </p>
      ...
      <section className="min-w-0 flex-1 space-y-5">
        <div className="rounded-lg border border-black/[0.06] bg-white p-4 shadow-sm dark:border-white/[0.04] dark:bg-white/[0.03]">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => { setEditingGist(null); setIsEditorOpen(true); }}
                className="inline-flex items-center gap-2 rounded-lg bg-brand-indigo px-3 py-2 text-sm font-medium text-white"
              >
        ...
        {gistSearchResults.length > 0 ? (
          <div className="grid gap-4">
            {gistSearchResults.map(gist => (
              <GistCard
                ...
                onOpen={openDetail}
                onEdit={(item) => { setEditingGist(item); setIsEditorOpen(true); }}
                onDeleted={(gistId) => { deleteGist(gistId); if (detailGist?.id === gistId) setIsDetailOpen(false); }}
                onUnstarred={(gistId) => {
          ...
          <div className="rounded-lg border border-dashed border-black/[0.08] bg-white p-12 text-center text-gray-500 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-text-tertiary">
            {t('暂无 gist。点击同步获取数据。', 'No gists yet. Sync to fetch data.')}
          </div>
        )}
      </section>

      <GistDetailModal gist={detailGist} isOpen={isDetailOpen} onClose={() => setIsDetailOpen(false)} />
      <GistEditorModal gist={editingGist} isOpen={isEditorOpen} onClose={() => setIsEditorOpen(false)} onSubmit={handleSubmitGist} />
    </div>
  );
};
