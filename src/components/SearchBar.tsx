...
import { syncStarredRepositories } from '../services/starSync';
          ...
          {sortOptions.map((option) => (
            <button
              ...
              className={`flex w-full items-center px-4 py-2 text-sm transition-colors ${
                value === option.value
                  ? 'bg-brand-indigo/[0.10] font-semibold text-brand-indigo dark:bg-brand-indigo/20 dark:text-white'
...
export const SearchBar: React.FC = () => {
  ...
  const filterChipBaseClass = 'flex items-center space-x-2 rounded-xl border px-3 py-2 text-xs font-semibold transition duration-200';
  const filterChipActiveClass = 'border-brand-indigo/25 bg-brand-indigo/[0.10] text-brand-violet shadow-sm dark:border-brand-indigo/30 dark:bg-brand-indigo/20 dark:text-indigo-200';
  const filterChipInactiveClass = 'border-slate-200/90 bg-white text-slate-500 hover:border-slate-300 hover:text-slate-950 dark:border-white/[0.08] dark:bg-white/[0.035] dark:text-slate-400 dark:hover:bg-white/[0.065] dark:hover:text-white';
  const filterTagBaseClass = 'rounded-xl border px-3 py-2 text-xs font-semibold transition duration-200';
  ...
  const handleAISearch = async () => {
    ...
    try {
      ...
      if (vsConfig?.enabled && activeEmbConfig && githubToken) {
        try {
  ...
  const handleStarSync = async () => {
    ...
    if (isSyncingStars) return;
    try {
      const { newRepoCount } = await syncStarredRepositories();
  ...
  return (
    <div className="gsm-panel-soft relative z-20 mb-5 p-3 sm:p-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          <button
            ...
            className={`flex h-9 items-center space-x-2 rounded-xl border px-3.5 text-xs font-semibold transition duration-200 ${
              showFilters || activeFiltersCount > 0
                ? 'border-brand-indigo/20 bg-brand-indigo/[0.10] text-brand-violet dark:bg-brand-indigo/20 dark:text-indigo-200'
                : 'border-slate-200/90 bg-white text-slate-500 shadow-sm hover:border-slate-300 hover:text-slate-950 dark:border-white/[0.08] dark:bg-white/[0.035] dark:text-slate-400 dark:hover:bg-white/[0.065] dark:hover:text-white'
            }`}
          >
      ...
      {/* Advanced Filters */}
      {showFilters && (
        <div className="mt-4 space-y-5 border-t border-slate-200/75 pt-4 dark:border-white/[0.07]">
          {/* Status Filters */}
          <div>
