...
type SettingsTab = 'general' | 'ai' | 'category' | 'menu' | 'data' | 'vectorSearch' | 'mcp';
...
const MobileTabNav: React.FC<MobileTabNavProps> = ({ tabs, activeTab, onTabChange }) => {
  ...
  return (
    <div 
      className="relative w-full border-b border-slate-200/75 bg-white/80 backdrop-blur-2xl dark:border-white/[0.07] dark:bg-[#101827]/90"
    >
      {/* 滚动容器 */}
      <div
      ...
      >
        {tabs.map((tab) => (
          <button
            ...
            className={`
              flex-shrink-0 flex items-center space-x-1.5 px-3 py-2 rounded-xl
              transition-all duration-150 ease-out snap-center
              min-h-[36px] touch-manipulation
              ${activeTab === tab.id
                ? 'bg-brand-indigo/[0.09] font-semibold text-brand-violet dark:bg-brand-indigo/20 dark:text-indigo-200'
                : 'text-slate-500 hover:bg-slate-100 hover:text-slate-950 dark:text-slate-400 dark:hover:bg-white/[0.055] dark:hover:text-white'
              }
            `}
            style={{
      ...
      {/* 底部活动指示器 */}
      <div
        className="absolute bottom-0 h-0.5 rounded-full bg-brand-violet transition-all duration-200 ease-out will-change-transform"
        style={{
...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({ 
  ...
  const tabs: SettingsTabItem[] = [
    ...
    { id: 'menu', label: t('菜单管理', 'Menu'), icon: <Layout className="w-5 h-5" /> },
    { id: 'data', label: t('数据管理', 'Data Management'), icon: <Trash2 className="w-5 h-5" /> },
    {
  ...
  if (isModal) {
    return (
      ...
      >
        <div className="flex h-[85vh] w-full max-w-5xl flex-col overflow-hidden rounded-[1.5rem] border border-slate-200/80 bg-white shadow-[0_40px_120px_rgba(15,23,42,0.28)] dark:border-white/[0.08] dark:bg-[#101827]">
          <div className="flex items-center justify-between border-b border-slate-200/75 px-6 py-4 dark:border-white/[0.07]">
          ...
          <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
            {/* 侧边栏 - 桌面端 */}
            <div className="hidden w-64 overflow-y-auto border-r border-slate-200/75 bg-slate-50/70 dark:border-white/[0.07] dark:bg-white/[0.02] md:block">
                ...
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors text-left ${
                      activeTab === tab.id
                        ? 'bg-brand-indigo/[0.10] font-semibold text-brand-violet dark:bg-brand-indigo/20 dark:text-indigo-200'
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:gap-5">
        {/* 桌面端侧边栏 */}
        <div className="hidden w-[248px] flex-shrink-0 lg:sticky lg:top-6 lg:block lg:self-start">
          <div className="gsm-panel-soft overflow-hidden p-2">
            <nav className="space-y-1" role="tablist" aria-label={t('设置标签页', 'Settings tabs')}>
              {tabs.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => handleTabChange(tab.id)}
                  ...
                  aria-selected={activeTab === tab.id}
                  aria-controls={`settings-tabpanel-${tab.id}`}
                  className={`flex w-full items-center space-x-3 rounded-xl px-3 py-2.5 text-left text-[13px] transition duration-200 ${
                    activeTab === tab.id
                      ? 'bg-brand-indigo/[0.10] font-semibold text-brand-violet ring-1 ring-brand-indigo/10 dark:bg-brand-indigo/20 dark:text-indigo-200'
                      : 'font-medium text-slate-500 hover:bg-slate-900/[0.045] hover:text-slate-950 dark:text-slate-400 dark:hover:bg-white/[0.055] dark:hover:text-white'
                  }`}
                >
                  {tab.icon}
                  <span className="font-medium">{tab.label}</span>
