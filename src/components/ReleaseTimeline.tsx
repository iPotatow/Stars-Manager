...
export const ReleaseTimeline: React.FC = () => {
    ...
    try {
      ...
      for (const entry of resolvedSources.entries) {
        ...
        if (entry.sources.includes(STARRED_RELEASE_SOURCE_ID)) {
          ...
          if (starredRepo) updateRepository({ ...starredRepo, has_fetched_releases: true, last_release_fetch_time: now });
        }
        if (entry.sources.includes(WATCH_CUSTOM_RELEASE_SOURCE_ID)) updateReleaseSourceRepository(WATCH_CUSTOM_RELEASE_SOURCE_ID, repo.full_name, { has_fetched_releases: true, last_release_fetch_time: now });
        if (entry.sources.includes(CUSTOM_RELEASE_SOURCE_ID)) updateReleaseSourceRepository(CUSTOM_RELEASE_SOURCE_ID, repo.full_name, { has_fetched_releases: true, last_release_fetch_time: now });
      }
      ...
      const updatedPart = updatedCount > 0
        ? (language === 'zh' ? `，${updatedCount} 个 Release 资产已更新` : `, ${updatedCount} release assets updated`)
        : '';
      if (failedRepos.length > 0) {
        message = language === 'zh'
          ? `刷新完成：发现 ${actuallyNewCount} 个新 Release${updatedPart}，${failedRepos.length} 个仓库刷新失败。`
          : `Refresh complete: found ${actuallyNewCount} new releases${updatedPart}; ${failedRepos.length} repositories failed.`;
      } else {
        message = language === 'zh'
          ? `刷新完成：发现 ${actuallyNewCount} 个新 Release${updatedPart}。`
          : `Refresh complete: found ${actuallyNewCount} new releases${updatedPart}.`;
      }
      ...
      const errorMessage = language === 'zh'
        ? 'Release 刷新失败，请检查网络连接。'
        : 'Release refresh failed. Check your network connection.';
  ...
  const handleUnsubscribeRelease = async (repoId: number) => {
    ...
    const sourceLabels = sourcesToRemove.map(sourceId => sourceId === STARRED_RELEASE_SOURCE_ID ? t('星标订阅', 'Starred subscriptions') : sourceId === WATCH_CUSTOM_RELEASE_SOURCE_ID ? t('Watch 仓库', 'Watch repositories') : t('自定义订阅', 'Custom subscriptions'));
    ...
    if (isOrphanRelease) {
      ...
      confirmMessage = language === 'zh' ? `确定取消订阅 "${releaseRepo.full_name}" 吗？确认后将一并取消 Watch 仓库来源。` : `Unsubscribe from "${releaseRepo.full_name}"? This will also remove it from Watch repositories.`;
    } else if (sourcesToRemove[0] === CUSTOM_RELEASE_SOURCE_ID) {
      confirmMessage = language === 'zh' ? `确定取消订阅 "${releaseRepo.full_name}" 吗？确认后将从自定义仓库列表中移除。` : `Unsubscribe from "${releaseRepo.full_name}"? This will remove it from the custom repository list.`;
    } else {
      confirmMessage = language === 'zh'
    ...
    if (sourcesToRemove.includes(WATCH_CUSTOM_RELEASE_SOURCE_ID)) removeReleaseSourceRepository(WATCH_CUSTOM_RELEASE_SOURCE_ID, releaseRepo.full_name);
    if (sourcesToRemove.includes(CUSTOM_RELEASE_SOURCE_ID)) removeReleaseSourceRepository(CUSTOM_RELEASE_SOURCE_ID, releaseRepo.full_name);
  ...
  if (subscribedReleases.length === 0) {
    ...
    return (
           ...
           <div className="mb-6 flex flex-col items-center gap-3">
             ...
             <div className="flex flex-wrap items-center justify-center gap-2">
               ...
               <button
                 ...
                 className="flex items-center space-x-2 px-4 py-3 bg-light-surface text-gray-700 rounded-lg hover:bg-gray-200 transition-colors"
               >
      ...
      <ReleaseSourceSettingsModal isOpen={isReleaseSourceSettingsOpen} onClose={() => setIsReleaseSourceSettingsOpen(false)} />
      </>
    );
  }
        ...
        <div className="flex flex-col gap-4 mb-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            <button
              type="button"
              onClick={() => setIsReleaseSourceSettingsOpen(true)}
              className="flex items-center space-x-2 px-3 py-2 bg-light-surface rounded-lg hover:bg-gray-200 transition-all"
            >
              <Settings className="w-4 h-4" />
              <span className="text-sm font-medium">{t('来源', 'Sources')}</span>
            </button>
          </div>
        </div>

        {/* Search and Filters */}
        <div className="gsm-panel-soft mb-4 p-3">
          {/* Search Bar */}
