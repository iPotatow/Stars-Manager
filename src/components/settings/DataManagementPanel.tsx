...
import type { DataSnapshot, DataSnapshotPayload } from '../../services/dataSnapshot';
import { parseDataSnapshot } from '../../services/dataSnapshot';
import {
...
const hasMaskedSecrets = (data: DataSnapshotPayload): boolean => (
...
export const DataManagementPanel: React.FC<DataManagementPanelProps> = ({ t }) => {
  ...
  const clearAllStorage = async () => {
    ...
    try {
      await indexedDBStorage.removeItem(PERSISTENCE_KEY);
      await indexedDBStorage.removeItem(LEGACY_PERSISTENCE_KEY);
    } catch (error) {
  ...
  const exportData = useCallback(async (selectedTypes: string[]) => {
    ...
    try {
      const store = useAppStore.getState();
      const exportDataObj: DataSnapshot = {
      ...
      const payload = exportDataObj.payload;

      if (selectedTypes.includes('repositories')) {
        payload.repositories = store.repositories;
      }
      if (selectedTypes.includes('releases')) {
        payload.releases = store.releases;
      }
      if (selectedTypes.includes('aiConfigs')) {
        payload.aiConfigs = store.aiConfigs.map(cfg => ({
          ...cfg,
          apiKey: cfg.apiKey ? MASKED_SECRET : '',
        }));
      }
      if (selectedTypes.includes('customCategories')) {
        payload.customCategories = store.customCategories;
        payload.hiddenDefaultCategoryIds = store.hiddenDefaultCategoryIds;
        payload.defaultCategoryOverrides = store.defaultCategoryOverrides;
        payload.categoryOrder = store.categoryOrder;
      }
      if (selectedTypes.includes('assetFilters')) {
        payload.assetFilters = store.assetFilters;
      }
      if (selectedTypes.includes('discoveryRepos')) {
        payload.discoveryRepos = store.discoveryRepos;
        payload.discoveryTotalCount = store.discoveryTotalCount;
        payload.discoveryHasMore = store.discoveryHasMore;
        payload.discoveryNextPage = store.discoveryNextPage;
      }
      if (selectedTypes.includes('subscriptionRepos')) {
        payload.subscriptionRepos = store.subscriptionRepos;
        payload.subscriptionLastRefresh = store.subscriptionLastRefresh;
        payload.subscriptionChannels = store.subscriptionChannels;
      }
      if (selectedTypes.includes('releaseSubscriptions')) {
        payload.releaseSubscriptions = Array.from(store.releaseSubscriptions);
        payload.releaseSourceSettings = store.releaseSourceSettings;
        payload.readReleases = Array.from(store.readReleases);
      }
      if (selectedTypes.includes('searchFilters')) {
        payload.searchFilters = store.searchFilters;
      }
      if (selectedTypes.includes('uiSettings')) {
        payload.language = store.language;
        payload.isSidebarCollapsed = store.isSidebarCollapsed;
        payload.releaseViewMode = store.releaseViewMode;
        payload.releaseSelectedFilters = store.releaseSelectedFilters;
        payload.releaseSearchQuery = store.releaseSearchQuery;
        payload.releaseExpandedRepositories = Array.from(store.releaseExpandedRepositories);
      }
      ...
      a.href = url;
      a.download = `stars-manager-data-${new Date().toISOString().split('T')[0]}.json`;
  ...
  const handleImportFile = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const data = parseDataSnapshot(JSON.parse(content));

        if (!data) {
          showError(t('无效的数据文件格式', 'Invalid data file format'));
          return;
        }

        setImportPreview({ data, isOpen: true, fileName: file.name });
      } catch {
        showError(t('解析文件失败，请确认文件是有效的 JSON。', 'Could not parse the file. Check that it contains valid JSON.'));
      }
    };
  ...
  const importData = useCallback(async (selectedTypes: string[], mode: 'merge' | 'replace') => {
    ...
    try {
      const store = useAppStore.getState();
      const importedData = importPreview.data.payload;
      ...
      if (mode === 'replace') {
        ...
        if (selectedTypes.includes('aiConfigs') && importedData.aiConfigs) {
          const restoredConfigs = importedData.aiConfigs.map(cfg => ({
            ...cfg,
            apiKey: isRealSecret(cfg.apiKey) ? cfg.apiKey : store.aiConfigs.find(c => c.id === cfg.id)?.apiKey || ''
          }));
        ...
        if (selectedTypes.includes('searchFilters') && importedData.searchFilters) {
          // Older snapshots may lack the licenses field; keep the current filter contract.
        ...
        if (selectedTypes.includes('aiConfigs') && importedData.aiConfigs) {
          ...
          const newConfigs = importedData.aiConfigs
            ...
            .map(cfg => ({
              ...cfg,
              apiKey: isRealSecret(cfg.apiKey) ? cfg.apiKey : ''
            }));
      ...
      if (hasMaskedSecrets(importedData)) {
        showSuccess(t(
          '数据已导入。部分密钥未包含在文件中，请在相应配置中重新输入。',
          'Data imported. Some secrets were not included in the file; re-enter them in the relevant settings.'
        ));
      } else {
  ...
  const getDeleteDescription = (type: DeleteOperation): string => {
    switch (type) {
      ...
      case 'all':
        return t(
          '将删除所有应用程序数据，包括用户数据、GitHub 令牌和配置文件。应用程序会重置为初始状态，且无法恢复。',
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        ...
        <p className="mb-4 text-sm text-gray-500 dark:text-text-tertiary">
          {t('导出的数据快照始终脱敏处理，不包含 API 密钥或其他凭据。', 'Data snapshots are always redacted and never include API keys or other credentials.')}
        </p>
      ...
      {cleanupSuggestions.length > 0 && (
        <section>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-text-primary mb-4 flex items-center">
            ...
            {t('可清理的数据', 'Data that can be removed')}
          </h3>
          <div className="bg-light-surface dark:bg-white/[0.04] border border-black/[0.06] dark:border-white/[0.04] rounded-lg p-4 mb-4">
            <p className="text-sm text-gray-700 dark:text-text-secondary ">
              {t('删除以下数据不会影响仓库和配置。', 'Removing the following data will not affect repositories or settings.')}
            </p>
          </div>
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              ...
              <p className="mt-2 text-gray-700 dark:text-text-secondary ">
                {t(
                  '此操作会永久删除所有应用程序数据，包括用户数据、GitHub 令牌和配置文件。应用程序会重置为初始状态，且无法恢复。',
          ...
          <div className="w-full max-w-lg bg-white dark:bg-panel-dark rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div className="text-sm text-gray-700 dark:text-text-tertiary">
                ...
                <p><strong>{t('创建日期:', 'Created:')}</strong> {new Date(importPreview.data.createdAt).toLocaleString()}</p>
                <p><strong>{t('应用版本:', 'App Version:')}</strong> {importPreview.data.appVersion}</p>
                <p><strong>{t('数据格式:', 'Data Format:')}</strong> v{importPreview.data.schemaVersion}</p>
              </div>
              ...
              <div className="border-t border-black/[0.06] dark:border-white/[0.04] pt-4">
                ...
                <div className="space-y-1 text-sm">
                  {importPreview.data.payload.repositories && (
                    <p className="text-gray-700 dark:text-text-tertiary">
                      • {t('仓库数据', 'Repositories')}: {importPreview.data.payload.repositories.length} {t('条', 'items')}
                    </p>
                  )}
                  {importPreview.data.payload.releases && (
                    <p className="text-gray-700 dark:text-text-tertiary">
                      • {t('Release数据', 'Releases')}: {importPreview.data.payload.releases.length} {t('条', 'items')}
                    </p>
                  )}
                  {importPreview.data.payload.aiConfigs && (
                    <p className="text-gray-700 dark:text-text-tertiary">
                      • {t('AI配置', 'AI Configs')}: {importPreview.data.payload.aiConfigs.length} {t('条', 'items')}
                    </p>
                  )}
                  {importPreview.data.payload.customCategories && (
                    <p className="text-gray-700 dark:text-text-tertiary">
                      • {t('分类设置', 'Categories')}: {importPreview.data.payload.customCategories.length} {t('条', 'items')}
                    </p>
                  )}
                  {importPreview.data.payload.assetFilters && (
                    <p className="text-gray-700 dark:text-text-tertiary">
                      • {t('资源过滤器', 'Asset Filters')}: {importPreview.data.payload.assetFilters.length} {t('条', 'items')}
                    </p>
                  )}
                </div>
              </div>
              ...
              {hasMaskedSecrets(importPreview.data.payload) && (
                <div className="flex items-start space-x-3 text-yellow-700 dark:text-yellow-500 bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg border border-yellow-200 dark:border-yellow-800">
                  ...
                  <p className="text-sm">
                    {t(
                      '提示：此数据文件中的部分密钥已被脱敏。导入后请在相应配置中重新输入密钥。',
                      'Some secrets in this data file are redacted. Re-enter them in the corresponding configurations after import.'
                    )}
                  </p>
                </div>
              )}
              ...
              <div className="flex space-x-3 pt-4">
                ...
                <button
                  onClick={() => {
                    const types = Object.keys(importPreview.data!.payload).filter(k => importPreview.data!.payload[k as keyof typeof importPreview.data.payload] !== undefined);
