...
export const AIConfigPanel: React.FC<AIConfigPanelProps> = ({ t }) => {
  ...
  const handleTest = async (config: AIConfig) => {
    ...
    try {
      ...
      if (result.success) {
        toast(t('AI 服务连接成功', 'AI service connected'), 'success');
      } else {
        toast(result.message, 'error');
      }
    } catch (error) {
      console.error('AI test failed:', error);
      toast(t('AI 服务测试失败，请检查网络和配置。', 'AI service test failed. Check the network and configuration.'), 'error');
    } finally {
      setTestingId(null);
    }
  };
  ...
  const handleTestForm = async () => {
    if (!form.baseUrl || !form.apiKey || !form.model) {
      toast(t('请先填写 API 端点、API 密钥和模型名称', 'Enter the API endpoint, key, and model first'), 'error');
      return;
    }
...
{REPO_INFO}`;
  ...
  const handleUseCustomPromptChange = useCallback((checked: boolean) => {
    ...
    if (checked) {
      ...
      if (form.customPrompt.trim() === '') {
        showNotification('info', t('已填入默认提示词，可以继续修改。', 'The default prompt was inserted. You can edit it.'));
      }
    } else {
      setShowCustomPrompt(false);
    }
  ...
  const handleToggleDefaultPrompt = useCallback(() => {
    if (showCustomPrompt) {
        showNotification('info', t('请先关闭自定义提示词编辑区。', 'Close the custom prompt editor first.'));
      return;
    }
  ...
  const handleRestoreDefaultPrompt = useCallback(async () => {
    ...
    if (isCustomPromptModified) {
      const confirmed = await confirm(
        t('恢复默认提示词？', 'Restore the default prompt?'),
        t('当前修改会被覆盖。', 'Your current changes will be replaced.'),
        { type: 'warning' }
      );
      if (!confirmed) return;
    }
