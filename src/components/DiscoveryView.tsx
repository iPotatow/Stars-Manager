...
import { getDefaultCategoryNames } from '../constants/repositoryCategories';
...
const MobileTabNav: React.FC<MobileTabNavProps> = ({ 
  ...
  return (
    <div 
      className="relative w-full border-b border-black/[0.06] bg-light-bg/95 backdrop-blur-sm dark:border-white/[0.04] dark:bg-panel-dark/95 lg:hidden"
    >
      <div
          ...
          {platforms.map((p) => (
            <button
              ...
              onClick={() => {
                ...
                platform === p.id
                  ? 'bg-brand-indigo/[0.15] text-brand-indigo dark:bg-brand-indigo/20 dark:text-white'
...
export const DiscoveryView: React.FC = React.memo(() => {
  ...
  const handleAnalyzePage = useCallback(async () => {
    ...
    if (unanalyzed.length === 0) {
      toast(t('已加载的项目都分析过了。', 'All loaded projects are already analyzed.'), 'info');
      return;
    }
    ...
    const categoryNames = [
      ...allCategoryNames,
      ...getDefaultCategoryNames(language),
    ];
    ...
    try {
      ...
      toast(t('AI 分析失败，请检查配置。', 'AI analysis failed. Check your configuration.'), 'error');
    } finally {
    ...
    <div className="flex flex-col">
      ...
      >
        ...
        <div className="flex-1 flex flex-col min-h-0 min-w-0 relative">
          ...
          >
            <div className="gsm-panel-soft mb-4 p-3.5 sm:p-4">
              {/* 第一行：标题和刷新按钮 */}
        ...
        {selectedDiscoveryChannel === 'topic' && (
                  ...
                  >
                    ...
                    <option value="web">{t('Web 开发', 'Web')}</option>
          ...
          >
            {selectedDiscoveryChannel === 'search' && (
              <div className="bg-white/80 dark:bg-panel-dark/80 backdrop-blur-xl rounded-2xl border border-black/[0.06] dark:border-white/[0.04] p-5 space-y-4 shadow-sm shadow-gray-200/50 dark:shadow-gray-900/20">
            ...
            {currentIsLoading && allRepos.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 gap-4">
                <div className="relative">
                  ...
                  <div className="absolute -right-1 -top-1 h-4 w-4 animate-ping rounded-full bg-status-emerald opacity-75" />
                </div>
                ...
                {selectedDiscoveryChannel === 'search' ? (
                  ...
                  <>
                    ...
                    <button
                      ...
                      className="px-6 py-2.5 rounded-xl bg-white dark:bg-white/[0.04] border border-black/[0.06] dark:border-white/[0.04] text-white hover:from-blue-600 hover:to-indigo-700 shadow-md shadow-blue-500/25 hover:shadow-lg transition-all duration-200 flex items-center gap-2 text-sm font-medium"
                    >
            ...
            {/* Page Info */}
            {!currentIsLoading && allRepos.length > 0 && (
              <div className="flex items-center justify-between py-3.5 px-5 bg-gradient-to-r from-gray-50 to-slate-50 dark:from-gray-800/60 dark:to-slate-800/40 rounded-xl border border-black/[0.04] dark:border-white/[0.02] text-sm">
