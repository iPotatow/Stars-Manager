      ...
      <div className="space-y-4">
        ...
        {hasAiWarning && (
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-3">
            <p className="text-sm text-amber-700 dark:text-amber-300 flex items-start">
              ...
              <span>
                {t(
                  '部分选中仓库尚未进行 AI 分析。还原到 AI 来源后，这些仓库会回退到默认来源。',
        ...
        {hasOriginalTargetAiLoss && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3">
            <p className="text-sm text-red-700 dark:text-red-300 flex items-start">
              ...
              <span>
                {t(
                  `还原到默认来源会清除 ${stats.hasAnyAiData} 个仓库的 AI 摘要和标签，且无法撤销。需要保留这些数据时，请选择“还原到 AI 来源”。`,
                  `Restoring to the default source clears AI summaries and tags for ${stats.hasAnyAiData} repositories. This cannot be undone. Choose "Restore to AI" to keep them.`
                )}
              </span>
            </p>
          </div>
        ...
        <div className="bg-gray-50 dark:bg-gray-700/30 border border-gray-100 dark:border-gray-600/50 rounded-lg p-3">
          <p className="text-xs text-gray-500 dark:text-gray-400 flex items-start">
            ...
            <span>
              {t(
                '还原到默认来源会清除 AI 摘要和标签，并显示 GitHub 原始内容。还原到 AI 来源只清除自定义内容。还原分类时会解除分类锁定。',
                'Restoring to the default source clears AI summaries and tags and shows the original GitHub content. Restoring to AI clears only custom content. Restoring categories also unlocks them.'
              )}
            </span>
          </p>
        </div>
