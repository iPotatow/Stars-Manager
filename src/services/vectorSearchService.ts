...
export class EmbeddingClient {
  ...
  async embed(texts: string[], purpose: 'document' | 'query' = 'document', signal?: AbortSignal): Promise<number[][]> {
    return backend.proxyEmbedding(this.config.id, texts, purpose, signal);
  }
...
export class VectorSearchService {
  private readonly baseUrl = '/api/vector';

  constructor(_config: VectorSearchConfig) {
    void _config;
  }
  ...
  private async request<T>(path: string, options: RequestInit = {}, signal?: AbortSignal): Promise<T> {
    ...
    const response = await fetch(url, { ...options, credentials: 'same-origin', headers, signal });
